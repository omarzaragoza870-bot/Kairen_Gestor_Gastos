...
export default function Metas() {
  ...
  const handleGuardar = async (form) => {
    ...
    try {
      if (form.id) {
        await editarMeta({ id: form.id, userId, nombre: form.nombre, montoObjetivo: form.montoObjetivo, montoActual: form.montoActual, fechaLimite: form.fechaLimite })
      } else {
        await crearMeta({ userId, nombre: form.nombre, montoObjetivo: form.montoObjetivo, montoActual: form.montoActual, fechaLimite: form.fechaLimite })
      }
      setEditando(null)
      await cargar(userId)
    } catch (err) {
        ...
        return (
          <div key={meta.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15 }}>{meta.nombre}</div>
  ...
  return (
    <div onClick={onCancelar} className="modal-backdrop">
      <div onClick={(e) => e.stopPropagation()} className="modal-card" style={{ maxWidth: 400 }}>
        <h3>{meta.id ? 'Editar meta' : 'Nueva meta'}</h3>

        <label className="field-label">Nombre de la meta</label>
        <div className="input-shell">
          <input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej. Vacaciones, Fondo de emergencia…" maxLength={60} />
        </div>
        ...
        <label className="field-label">Cuánto llevas ahorrado</label>
        <div className="input-shell">
          <span style={{ color: 'var(--text-muted)' }}>$</span>
        ...
        <div className="modal-actions" style={{ marginTop: 4 }}>
          ...
          <button
            ...
            onClick={() => onGuardar({ id: meta.id, nombre: nombre.trim(), montoObjetivo: objetivoNum, montoActual: actualNum, fechaLimite })}
